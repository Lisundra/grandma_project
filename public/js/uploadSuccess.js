...
uploadBtn?.addEventListener('click', async (event) => { //! ? - оператор опциональной последовательности
  ...
  try {
    ...
    textBlock.append(soundBtnContainer);

    const soundBtnElement = document.createElement('button');
    soundBtnElement.className = 'sound-btn';
    soundBtnElement.type = 'button';
    soundBtnElement.innerHTML = 'ОЗВУЧИТЬ ТЕКСТ';
    soundBtnContainer.append(soundBtnElement);
